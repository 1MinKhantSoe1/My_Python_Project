...
def main():
    ...
    def DownloadVideo():
        ...
        if len(url) > 1:
            ...
            try:
                ...
                ydError.config(text="Somethings Wrong Please Check Step by Step Carefully !!", fg="red",font=("jost",8))


        try:

            select.download(Location_Name)
            ydError.config(text="Successfully Downloaded!!", fg="green")

        except UnboundLocalError:

            ydError.config(text="Somethings Wrong Please Check Step by Step Carefully !!", fg="red")



    tk = Tk()

    tk.title("Youtube Video Downloader")
    tk.geometry("350x400")
    ...
    DeveloperNameLabel = Label(tk, text="Min Khant Soe", font=("jost", 12))
